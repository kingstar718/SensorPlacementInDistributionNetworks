#!/usr/bin/env python
#encoding:UTF-8
from numpy import array, max, min, hstack, arange, sum, argsort
...
def crowddist(funScore, indicate):
    # 求出该层　评分子矩阵
    indicate = array(indicate)     # 生成矩阵[1 3 7]
    score =funScore[indicate]       # 选择funScore的第 1 3 7 列

    # 求出集合中 不同属性的 范围
    # rangeVector=np.array([1.0, 1.0])
    maxVector = max(funScore, axis=0)      # 求funScore的最大值
    minVector = min(funScore, axis=0)
    ...
    indicateVector = arange(N)     # 生成从0开始到N的[] [0 1 2]
    indicateVectorT = indicateVector.reshape(N, 1)  # 改变数组形状 [[0] [1] [2]]

    # 生成 函数值和个体序号 矩阵
    dist=array([0.0]*N*V).reshape(N, V)      # [[ 0.  0.] [ 0.  0.] [ 0.  0.]]
    scoreIndicateMatrix = hstack((score, indicateVectorT))  # [[2 3 0]  [3 2 1]  [3 2 2]]
    ...
    distVector = sum(dist/rangeVector, axis=1)    # 后加了双斜线
...
if __name__ == "__main__":
    funScore=array([[1,2], [2,3], [2,2],[2,2],[2,2],[2,2],[2,2],[2,2],[2,2],[2,2], [3,2], [4,3], [2,1], [3,1], [3,2], [3,3], [3,4]])
